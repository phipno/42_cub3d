...
/*   Updated: 2023/06/05 13:37:15 by jwillert         ###   ########          */
/*                                                                            */
/* ************************************************************************** */
		...
		double max_line)
{
	...
	scale = get_bigger_sizet(max_column, max_line),
	minimap->offset_x = WIDTH / 100 * 15;
	minimap->offset_y = HEIGHT / 100 * 15;
	point_set(&minimap->content_start, minimap->offset_x, minimap->offset_y);
	point_set(&minimap->content_end, WIDTH - minimap->offset_x,
		HEIGHT - minimap->offset_y);
	size_x = (minimap->content_end.x - minimap->content_start.x) / scale;
	size_y = (minimap->content_end.y - minimap->content_start.y) / scale;
	element_set(&minimap->element, size_x, size_y);
	...
	point_set(&minimap->border_end, WIDTH - minimap->offset_x,
		HEIGHT - minimap->offset_y);
}
